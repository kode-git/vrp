...
\begin{enumerate}
 \setcounter{enumi}{2}
    ...
    \item \begin{math}s_{i} = j \Rightarrow p_{j} = i, \forall i,j in \{1,...,N + V * 2\}\end{math}
...
\begin{enumerate}
 \setcounter{enumi}{9}
    \item \begin{math}lex([s_{1},...,s_{NumVisits}],[p_{1},...,p_{NumVisits}])\end{math} breaks some routes symmetry when, for each vehicles, the route starts from the last customers visited by a vehicle and goes back to the first one with the same total distance of the respectively symmetrical route.
